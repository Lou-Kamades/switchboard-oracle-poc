...
pub async fn fetch_all_oracles(rpc_client: &RpcClient) -> Result<Vec<OracleData>> {
    ...
    let account = rpc_client
        .get_account_with_config(&oracle_key, account_config)
        .await?;
...
pub async fn fetch_oracles_by_name(
    rpc_client: &RpcClient,
    names: Vec<String>,
) -> Result<Vec<OracleData>> {
    let oracles = fetch_all_oracles(rpc_client).await?;

    let buffers: Vec<[u8; 16]> = names
        .into_iter()
        .map(|n| {
            let mut oracle_buffer = [0u8; 16];
    ...
    let oracles: Vec<OracleData> = oracles
        .into_iter()
        .filter(|x| buffers.contains(&x.name))
        .collect();
    Ok(oracles)
}
